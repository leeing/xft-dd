...
from diligence.models import DimensionSearchResult, DimensionSummary, RunMeta
...
def save_node(state: DiligenceState) -> dict:
    ...
    status = "success" if not failed_dims and report else ("partial" if report else "failed")
    ...
    log.info("artifacts_saved", output_dir=str(output_dir), status=status)
