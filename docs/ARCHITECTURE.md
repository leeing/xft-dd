...
本文档面向开发者，记录 XFT 的架构、数据流、模块边界、证据模型、Web 缓存、批量运行和工程化细节。

业务人员使用说明请看根目录 [README.md](../README.md)。
...
data/ Prophet/NewEnt JSON
  → xft warehouse build            # 本地 JSON 分层入库
...
```text
xft web enrich                     # 搜索 → 抓取 → LLM 抽取 → 缓存
  → xft web import                 # Web 证据入库
  → xft recommend --with-web-evidence
```
...
`uv run xft warehouse build` 对 `data/` 做分层入库：
...
```bash
uv run xft web enrich --refresh-search       # 仅重搜
uv run xft web enrich --refresh-fetch        # 仅重抓
uv run xft web enrich --refresh-extraction   # 仅重抽
uv run xft web enrich --extract-only --source-run-id web_xxx  # 从已有搜索结果重放抽取
```
...
```bash
uv run xft recommend --scenario config/scenarios/sales_recommendation "企业名称"
uv run xft web enrich --scenario config/scenarios/sales_recommendation "企业名称"
```
...
recommendation_runs/batches/{batch_id}/
  ...
  runs/{company}/             # 每家企业独立目录
    report.md
    result.json
    config_manifest.json      # 本次运行使用的配置文件与内容 hash
    scenario_resolved.json    # 场景解析结果与有效产品配置 hash
...
`delivery_manifest.json` 会引用每家企业的 `config_manifest.json` 和 `scenario_resolved.json`。复现某次交付时，优先查看对应 run 目录下的 `config_manifest.json`：其中记录了 products、dimensions、Web、LLM、scoring policy、evidence policy、prompt 的路径和 sha256，以及 patch 后的 effective hash。
...
```mermaid
sequenceDiagram
  participant CLI as xft web enrich
  participant DB as DuckDB
...
```text
uv run xft warehouse build         # data/ JSON -> DuckDB
uv run xft web enrich              # Web 搜索、抓取、抽取、缓存
...
uv run xft runs inspect            # 检查已生成 run 结果
uv run xft cache sync-remote       # 远程缓存同步到本地 DuckDB

src/xft/                           # 平台根包（规范包名）
src/xft/cli/                       # 统一 CLI 入口实现
...
```bash
uv run xft web enrich --refresh-search "企业名称"
uv run xft web enrich --refresh-extraction "企业名称"
uv run xft web enrich --extract-only --source-run-id web_xxx "企业名称"
```

从 Web 缓存重建 DuckDB Web 表：

```bash
uv run xft web import --input data/web --warehouse cache/company_warehouse.duckdb --rebuild
```
...
```bash
uv run xft calibrate --limit 10 --batch-id calibration-run-01
uv run xft calibrate --labels calibration_labels.csv --limit 30
```
...
`--scenario` 是推荐给业务人员使用的主入口。它会同时切换产品、维度、prompt、Web 配置、评分策略、证据策略、推荐输出目录和 Web cache root。新增业务场景时，优先使用 `extends` 继承已有场景，再用 `patches.products` 局部调整产品规则，避免复制整份 `products.yaml`。
...
场景可以只声明差异。例如 `config/scenarios/bank_marketing/scenario.yaml` 继承销售推荐场景，只调整 CRM 产品：
...
- `set`：修改 `module_name`、`priority`、`base_score`、`target_needs`、`match_rule`
- `append_positive_rules` / `append_negative_rules` / `append_exclusion_rules`
- `replace_positive_rules` / `replace_negative_rules` / `replace_exclusion_rules`，按规则 `id` 替换
- `remove_positive_rules` / `remove_negative_rules` / `remove_exclusion_rules`，按规则 `id` 删除

运行推荐时，run 目录会写入 `scenario_resolved.json`，其中包含 patch 后的 `products_effective_hash` 和产品数量，便于复现本次报告。
...
- 场景可继承：`extends` / `overrides` / `patches.products` 减少跨场景复制，`scenario_resolved.json` 可审计。
...
- 第二个真实业务场景落地：用产品 patch 验证多场景配置长期可维护性。
- 交付包增强：增加 `.xlsx` 汇总交付和 `.zip` 交付包。
